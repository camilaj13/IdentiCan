...
def draw_tab_bar(draw, active_tab=0):
    ...
    tabs = [("My Dogs", True if active_tab == 0 else False),
            ("Verifier", True if active_tab == 1 else False)]
...
def create_login_screen():
    ...
    f_emoji = font(48)
    draw.text((W//2, y), "O", fill=PRIMARY, font=f_emoji, anchor="mm")
    ...
    y += 48*SCALE
    f_sub = font(13)
    sub = "Canine Biometric Identification"
    ...
    y_px += 64
    draw_input(draw, 30, y_px, 315, "Password", "********")

    y_px += 80
    draw_button(draw, 30, y_px, 315, 50, "Sign In", PRIMARY)
...
def create_home_screen():
    ...
    draw.text((20*SCALE, gy + 4*SCALE), "Hello, Maria", fill=TEXT, font=font(18, bold=True))
    draw.text((20*SCALE, gy + 32*SCALE), "3 registered dog(s)", fill=TEXT_SEC, font=font(12))

    cy = gy + 72*SCALE

    # Dog cards
    dogs = [
        ("Luna", "Poodle", "F", "3 yrs", "5.2 kg", "IDC-DOG-00001", "#E3F2FD"),
        ("Rocky", "Labrador", "M", "5 yrs", "28.0 kg", "IDC-DOG-00002", "#FFF3E0"),
        ("Coco", "Mixed", "M", "2 yrs", "12.5 kg", "IDC-DOG-00003", "#E8F5E9"),
    ]
...
def create_profile_screen():
    ...
    draw.text((tx, cy + 52*SCALE), "Poodle", fill=TEXT_SEC, font=font(14))
    ...
    details = [
        ...
        ("WEIGHT", "5.2 kg"), ("COLOR", "White"),
        ("ORIGIN", "Adopted"), ("MICROCHIP", "-"),
    ]
    ...
    draw_button(draw, 16, (cy + 300*SCALE)/SCALE, half_w/SCALE, 44, "Vaccines", PRIMARY)
    draw_button(draw, 16 + half_w/SCALE + 16, (cy + 300*SCALE)/SCALE, half_w/SCALE, 44, "QR Code", SECONDARY)

    reg_y = btn_y + 64*SCALE
    draw.text((W//2, reg_y), "Registered on 02/16/2026", fill=TEXT_SEC, font=font(11), anchor="mm")
...
def create_vaccines_screen():
    ...
    by = draw_app_header(draw, "Luna's Vaccines", back=True)
    ...
    vaccines = [
        ("Rabies", "06/15/2025", "Dr. Garcia"),
        ("DHPP", "03/10/2025", "Dr. Lopez"),
    ]
    ...
    cy += 20*SCALE
    draw.text((W//2, cy), "2 vaccines recorded", fill=TEXT_SEC, font=font(12), anchor="mm")
...
def create_qr_screen():
    ...
    by = draw_app_header(draw, "QR Code", back=True)
    ...
    inst = "Scan this QR code to\nidentify your pet"
    draw.text((W//2, inst_y), inst, fill=TEXT_SEC, font=font(12), anchor="mm", align="center")

    btn_y = (cy + card_h + 24*SCALE) / SCALE
    draw_button(draw, 24, btn_y, 155, 44, "Share", PRIMARY)
    draw_button(draw, 195, btn_y, 155, 44, "Download PDF", PRIMARY, outlined=True)
...
def create_scan_nose_screen():
    ...
    by = draw_app_header(draw, "Verifier")
    ...
    draw.text((W//2, cy+128*SCALE), "Scan Nose", fill=TEXT, font=font(20, bold=True), anchor="mm")
    ...
    draw_button(draw, 16, btn_base, 343, 52, "Start Scanning", PRIMARY)
    draw_button(draw, 16, btn_base + 64, 343, 44, "Scan QR Instead", PRIMARY, outlined=True)
...
def create_result_screen():
    ...
    draw.text((W//2, cy + 90*SCALE), "Match Found", fill=TEXT, font=font(18, bold=True), anchor="mm")
    draw.text((W//2, cy + 120*SCALE), "A registered dog was found", fill=TEXT_SEC, font=font(12), anchor="mm")
    ...
    tx = card_margin + 20*SCALE
    draw.text((tx, cy2 + 16*SCALE), "Dog Details", fill=PRIMARY, font=font(14, bold=True))
    draw.line([tx, cy2 + 42*SCALE, card_margin + card_w - 20*SCALE, cy2 + 42*SCALE], fill=BORDER, width=1)

    details = [("Name", "Luna"), ("ID", "IDC-DOG-00001"), ("Confidence", "95.8%")]
    dy = cy2 + 56*SCALE
    ...
    draw.text((card_margin + 16*SCALE, cy3 + 12*SCALE), "Daily Usage", fill=SECONDARY, font=font(12, bold=True))
    draw.text((card_margin + 16*SCALE, cy3 + 36*SCALE), "You have 2 verifications left today", fill=TEXT_SEC, font=font(11))

    btn_y_val = (cy3 + 80*SCALE) / SCALE
    half = 163
    draw_button(draw, 16, btn_y_val, half, 44, "New Verification", PRIMARY)
    draw_button(draw, 16 + half + 16, btn_y_val, half, 44, "Go Back", PRIMARY, outlined=True)
...
def create_add_dog_screen():
    ...
    by = draw_app_header(draw, "Add Dog", back=True)

    cy = by + 16*SCALE
    tx = 16*SCALE

    draw.text((tx + 4*SCALE, cy), "Basic Info", fill=PRIMARY, font=font(16, bold=True))
    cy += 32*SCALE

    draw_input(draw, 16, cy/SCALE, 343, "Dog name *", "Luna")
    cy += 64*SCALE

    draw_input(draw, 16, cy/SCALE, 343, "Breed", "Poodle")
    cy += 64*SCALE

    draw.text((tx + 4*SCALE, cy), "Sex", fill=TEXT_SEC, font=font(11))
    cy += 22*SCALE
    seg_w = 170*SCALE
    draw.rounded_rectangle([tx, cy, tx+seg_w, cy+38*SCALE], radius=8*SCALE, outline=BORDER, width=2)
    draw.text((tx + seg_w//2, cy+19*SCALE), "Male", fill=TEXT_SEC, font=font(13), anchor="mm")
    draw.rounded_rectangle([tx+seg_w, cy, tx+seg_w*2+4*SCALE, cy+38*SCALE], radius=8*SCALE, fill=PRIMARY)
    draw.text((tx+seg_w + seg_w//2 + 2*SCALE, cy+19*SCALE), "Female", fill=WHITE, font=font(13, bold=True), anchor="mm")
    cy += 52*SCALE

    draw.text((tx + 4*SCALE, cy), "Origin", fill=TEXT_SEC, font=font(11))
    cy += 22*SCALE
    origins = ["Adopted", "Purchased", "Rescued", "Other"]
    ox = tx
    ...
    half_w = 165
    draw_input(draw, 16, cy/SCALE, half_w, "Age (years)", "3")
    draw_input(draw, 16 + half_w + 13, cy/SCALE, half_w, "Weight (kg)", "5.2")
    cy += 64*SCALE

    draw_input(draw, 16, cy/SCALE, 343, "Color", "White")
    cy += 80*SCALE

    draw_button(draw, 16, cy/SCALE, 343, 50, "Register Dog", PRIMARY)
...
def compose_screens():
    screens = [
        ...
        ("My Dogs", create_home_screen()),
        ("Add Dog", create_add_dog_screen()),
        ("Profile", create_profile_screen()),
        ("Vaccines", create_vaccines_screen()),
        ("QR Code", create_qr_screen()),
        ("Verifier", create_scan_nose_screen()),
        ("Result", create_result_screen()),
    ]
    ...
    total_h = rows * (H + label_h) + (rows + 1) * padding + 80*SCALE
    ...
    sub_f = font(14)
    draw.text((total_w//2, 72*SCALE), "Canine Biometric Identification  |  React Native + FastAPI", fill="#aaaaaa", font=sub_f, anchor="mm")
